...
public:
    ...
    //*************************************************************
    int solve(double solvedPerc, double currPerc) {
        char* ptr;
            ...
            if (t == 0) {
                ...
                for (int i = 0; i < numRows; i++)
                    if (rows_size[i] == 1)
                    {
                        ...
                        return simple.solve(solvedPerc, currPerc);
                    }
            }
    ...
    int solveRec(double solvedPerc, double currPerc ) {
        char* ptr;
            ...
            if (t == 0)
            {
                ...
                for (int i = 0; i < numRows; i++)
                    if (rows_size[i] == 1)
                    {
                        ...
                        BBNode simple( *this, rows[i][0]);
                        //printf("Entrou aki\n");
                        return simple.solveRec(solvedPerc, currPerc);
                    }
            }
        ...
        for (int j = 0; j < branch_size; j++)
        {
            ...
            double chPerc = currPerc / double(branch_size);
            nodeCount += child.solveRec(acumPerc, chPerc );
            acumPerc += chPerc;
...
int main( int argc, char* argv[] ) {
    ...
    if (parallel){
    	root.solve(0.0, 100.0);

    	pthread_t *threads = (pthread_t*) malloc(sizeof(pthread_t) * index);

	    for (int i = 0; i < index; i++){
	    	pthread_create(&threads[i], NULL, run, (void*) nodeVetor[i].child);
	    }
...
void *run(void *args){
	...
	BBNode *param = (BBNode*) args;
	param->solveRec(0.0, 100.0);
